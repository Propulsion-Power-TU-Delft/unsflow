...
r1 = 1  # inner radius [m]
r2 = 2  # outer radius [m]
p1 = 101.3e3  # pressure at diffuser inlet [Pa]
T = 288  # temperature [K]
b = (r2-r1)/10  # diffuser width [m]
R = 287.058  # air gas constant [kJ/kgK]
gmma = 1.4  # cp/cv ratio of air
rho1 = p1/R/T  # density [kg/m3]
...
# %%%%%%%%%%%%%%%%%%%%%%% COMPUTATIONAL PART %%%%%%%%%%%%%%%%%%%%%%%
Nst = 50  # streamwise points
Nsp = 5  # spanwise points

folder_path = "pictures/%02i_%02i" %(Nst, Nsp)  # Replace with the desired folder path
...
fig.savefig('pictures/%02i_%02i/chi_map_arnoldi.pdf' % (Nst, Nsp), bbox_inches='tight')
...
for ivec in range(np.shape(eigenvectors)[1]):
    ...
    def scaled_eigenvector_real(eig_list):
        ...
        array = np.reshape(array, (Nst, Nsp))
    ...
    xtick_locations = [0, b/config.get_reference_length()]
    xtick_labels = [r'$0$', r'$b$']
    ...
    plt.savefig('pictures/%02i_%02i/eigenfunction_rho_%i.pdf' % (Nst, Nsp, ivec + 1), bbox_inches='tight')
    ...
    cnbar = plt.colorbar(cnt)
    cnbar.ax.tick_params(labelsize=font_axes)
    plt.savefig('pictures/%02i_%02i/eigenfunction_ur_%i.pdf' % (Nst, Nsp, ivec + 1), bbox_inches='tight')
    ...
    cnbar = plt.colorbar(cnt)
    cnbar.ax.tick_params(labelsize=font_axes)
    plt.savefig('pictures/%02i_%02i/eigenfunction_ut_%i.pdf' % (Nst, Nsp, ivec + 1), bbox_inches='tight')
    ...
    cnbar = plt.colorbar(cnt)
    cnbar.ax.tick_params(labelsize=font_axes)
    plt.savefig('pictures/%02i_%02i/eigenfunction_uz_%i.pdf' % (Nst, Nsp, ivec + 1), bbox_inches='tight')
    ...
    cnbar = plt.colorbar(cnt)
    cnbar.ax.tick_params(labelsize=font_axes)
    plt.savefig('pictures/%02i_%02i/eigenfunction_p_%i.pdf' % (Nst, Nsp, ivec + 1), bbox_inches='tight')


file_path = 'data/meta/%02i_%02i_%02i.pickle'%(Nst, Nsp, config.get_grid_transformation_gradient_order())
