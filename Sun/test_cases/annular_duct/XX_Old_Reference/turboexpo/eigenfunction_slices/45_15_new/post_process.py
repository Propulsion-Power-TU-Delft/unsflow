...
LAMBDA = roots[1]
k_1 = 1/(1-M**2) * (omega*M/a + np.sqrt((omega/a)**2 - (1-M**2)*LAMBDA**2))
k_2 = 1/(1-M**2) * (omega*M/a - np.sqrt((omega/a)**2 - (1-M**2)*LAMBDA**2))


print("k1: ", k_1)
k_3 = k_1-2*np.pi/L
z_grid_refined = np.linspace(z_grid[0,0], z_grid[-1, 0], 200)*x_ref

Z_eig = np.exp(1j*k_2*z_grid_refined) - np.exp(1j*k_3*z_grid_refined)

plt.figure()
plt.plot(z_grid_refined, Z_eig.real)
# plt.plot(z_grid_refined, Z_eig.imag)
