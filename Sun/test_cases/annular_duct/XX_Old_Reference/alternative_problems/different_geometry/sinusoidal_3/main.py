...
N_AXIAL_HALFWAVE = 3
AMPLITUDE = (r2-r1)*0.5
Kz = N_AXIAL_HALFWAVE*np.pi/L  # angle of aperture of the duct [rad]
...
z0 = duct_Obj1.z_cg.copy()
r0 = duct_Obj1.r_cg.copy()


# overwriting the baseflow and grid
duct_Obj1.r_cg = r0 + AMPLITUDE*np.sin(Kz*z0)
duct_Obj1.r_grid = duct_Obj1.r_cg.copy()
axial_area = np.zeros_like(z0)
for ii in range(Nz):
    for jj in range(Nr):
        axial_area[ii, jj] = np.pi*(duct_Obj1.r_cg[ii, -1]**2 - duct_Obj1.r_cg[ii, 0]**2)
mflow_in = axial_area[0,0]*duct_Obj1.uz[0,0]

duct_Obj1.z_grid = duct_Obj1.z_cg.copy()

duct_Obj1.uz = mflow_in/axial_area
duct_Obj1.ur = axialVel*AMPLITUDE*Kz*np.cos(Kz*duct_Obj1.z_cg)
u_mag_post = np.sqrt((duct_Obj1.uz**2 + duct_Obj1.ur**2))
pdyn = duct_Obj1.p[0,0]+0.5*rho*(duct_Obj1.ur[0,0]**2 + duct_Obj1.uz[0,0]**2)
duct_Obj1.p = pdyn - 0.5*rho*(u_mag_post**2)
duct_Obj1.duz_dz = -mflow_in/axial_area**2 * 2*AMPLITUDE*np.pi*Kz*np.cos(Kz*duct_Obj1.z_cg)*(r2-r1)
duct_Obj1.dur_dz = -axialVel*AMPLITUDE*Kz*np.sin(Kz*duct_Obj1.z_cg)
u_mag_post_prime = (duct_Obj1.ur*duct_Obj1.dur_dz + duct_Obj1.uz*duct_Obj1.duz_dz)/u_mag_post
duct_Obj1.dp_dz = -rho*u_mag_post*u_mag_post_prime
