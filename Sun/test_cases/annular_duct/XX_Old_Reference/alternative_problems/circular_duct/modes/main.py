...
for ivec in range(10):
    ...
    contour = ax[iax, jax].contourf(z_grid, r_grid, rho_eig_r, levels=N_levels_medium, cmap='bwr')
    if jax == 0:
        ax[iax, jax].set_ylabel(r'$r$ [-]', fontsize=font_labels)
    if iax==1:
        ax[iax, jax].set_xlabel(r'$z$ [-]', fontsize=font_labels)
    ax[iax, jax].set_xticks([])
    ax[iax, jax].set_yticks([])
    ax[iax, jax].set_title(r'$\tilde{\rho}_{%i}$' % (ivec + 1), fontsize=font_title)
plt.savefig('pictures/%02i_%02i/eigenfunction_rho_total.pdf' % (Nz, Nr), bbox_inches='tight')
