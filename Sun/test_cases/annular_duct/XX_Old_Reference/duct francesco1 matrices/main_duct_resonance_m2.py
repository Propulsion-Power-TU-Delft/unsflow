...
m = 2  # second circumferential mode


def Bessel_determinant(lmbda, r1=r1, r2=r2, m=2):
    r = np.linspace(r1, r2)
...
def lambda_root(lmbda_span, r1=r1, r2=r2, m=2):
    det = np.array(())
    for s in range(0, len(lambda_span)):
        lmbda = lambda_span[s]
        det = np.append(det, Bessel_determinant(lmbda, r1, r2, m=m))
    return det
...
det = lambda_root(lambda_span, m=m)
...
sun_obj.AddAMatrixToNodesFrancesco()
sun_obj.AddBMatrixToNodesFrancesco()
sun_obj.AddCMatrixToNodesFrancesco(m=m)
sun_obj.AddEMatrixToNodesFrancesco()
sun_obj.AddRMatrixToNodesFrancesco()
...
sun_obj.PlotInverseConditionNumber(ref_solution=omega_analytical, save_filename='chi_map_%s-%s_%d_%d_m%d'
