...
axial_orders = np.linspace(5, 5, 1)
...
for m in radial_orders:
    for alpha in axial_orders:
        omega_plus = (2 * OMEGA1 * (pi * alpha * D / L)) / sqrt(((pi * alpha * D / L) ** 2 + m ** 2 * pi ** 2))
        omega_minus = -(2 * OMEGA1 * (pi * alpha * D / L)) / sqrt(((pi * alpha * D / L) ** 2 + m ** 2 * pi ** 2))
...
sun_obj.L0 = np.eye(sun_obj.nPoints)*4*(OMEGA1/omega_ref)**2 * a_axial**2
sun_obj.L1 = np.zeros_like(sun_obj.L0)

xi = GaussLobattoPoints(sun_obj.data.nPoints)
D = ChebyshevDerivativeMatrixBayliss(xi)
D2 = D@D
sun_obj.L2 = 4 * D2 -np.eye(N)*a_axial**2
...
for eig in eigs_an_list:
    if kk == 0:
        plt.scatter(eig.omega_pos.real, eig.omega_pos.imag, marker='x', c='blue', s=marker_size, label='analytical')
    else:
...
fig.savefig('pictures/%02i/eigenvalue_map.pdf' % (N), bbox_inches='tight')
...
ax.legend(fontsize=font_legend)
ax.grid(alpha=grid_opacity)
fig.savefig('pictures/%02i/eigenvalue_map_zoom.pdf' % (N), bbox_inches='tight')
