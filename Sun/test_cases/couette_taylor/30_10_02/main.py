...
rho = 1.014  # density [kg/m3]
omega1 = 0  # omega inner cylinder
omega2 = 10  # omega outer cylinder
HARMONIC_ORDER = 1
...
for ii in range(0, Nz):
    for jj in range(0, Nr):
        density[ii, jj] = rho
        axialVel[ii, jj] = u_ref
        pressure[ii, jj] = p

config = Config('duct.ini')
duct_Obj1 = Sun.src.AnnulusMeridional(0, L, r1, r2, Nz, Nr, density, radialVel, tangentialVel, axialVel, pressure,
                                      config, mode='gauss-lobatto')
# override the coutte-taylor baseflow solution
duct_Obj1.ur = np.zeros((Nz, Nr))
A = (omega2*r2**2 - omega1*r1**2)/(r2**2-r1**2)
B = (omega1 - omega2)/(r1**(-2)-r2**(-2))
duct_Obj1.ut = A*duct_Obj1.r_grid + B/duct_Obj1.r_grid
duct_Obj1.uz = np.zeros((Nz, Nr))
