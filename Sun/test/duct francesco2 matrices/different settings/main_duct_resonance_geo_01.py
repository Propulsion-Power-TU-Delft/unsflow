...
r1 = 0.32  # inner radius [m]
r2 = 0.35  # outer radius [m]
M = 0.015  # Mach number
p = 100e3  # pressure [Pa]
T = 288  # temperature [K]
L = 0.08  # length [m]
...
def find_multiple_zeros(f, xmin, xmax, intervals=20):
    roots = []
...
ax.legend()
fig.savefig('pictures/lambda_roots_geo1_m%d.pdf' %(m), bbox_inches='tight')
...
omega_domain = [10e3, 35e3, -8e3, 8e3]
grid_omega = [150, 10]
sun_obj.ComputeSVD(omega_domain = omega_domain / omega_ref, grid_omega = grid_omega)
sun_obj.PlotInverseConditionNumber(ref_solution=omega_analytical, save_filename='chi_map_geo1_ur_%s-%s_%d_%d_m%d'
