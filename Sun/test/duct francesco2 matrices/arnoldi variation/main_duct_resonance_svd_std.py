...
sun_obj.PlotInverseConditionNumber(ref_solution=omega_analytical, save_filename='chi_map_svd_std_%d_%d' %(Nz, Nr))
#
# sun_obj.ComputeSVD2(omega_domain = omega_domain/omega_ref, grid_omega = grid_omega)
# sun_obj.PlotInverseConditionNumber(ref_solution=omega_analytical, save_filename='chi_map_svd2_%d_%d' %(Nz, Nr))


#
# sigma = 15000/omega_ref
# eigenvalues, eigenvectors = eigs(sun_obj.Z_g, M=sun_obj.A_g, sigma=sigma, k=300)
# eigenvalues *= omega_ref
# #
# marker_size=100
# fig, ax = plt.subplots(figsize=(10, 7))
# ax.scatter(eigenvalues.real, eigenvalues.imag, marker='o', facecolors='none', edgecolors='black',
#            s=marker_size, label=r'eigenfrequencies')
# # ax.scatter(eig_val.real, eig_val.imag, marker='x', facecolors='black', label=r'numerical', s=marker_size)
# ax.scatter(sigma.real*omega_ref, sigma.imag*omega_ref, marker='s', facecolors='red', edgecolors='red', label=r'$\sigma$ initial guess', s=marker_size)
