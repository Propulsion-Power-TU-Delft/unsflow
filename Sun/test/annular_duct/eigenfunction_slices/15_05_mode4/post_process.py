...
# # second axial order
plt.figure()
plt.plot(z_grid_refined, np.max(p_eig_r[:, 3]) * np.sin(2 * np.pi * z_grid_refined / L * r1), label='analytical',
         lw=medium_line_width)
plt.plot(z_grid[:, 3], p_eig_r[:, 3], 'ro', label='numerical', lw=medium_line_width, markerfacecolor='none',
