...
for k_idx in k:
    ...
    k_valve = k_idx  # valve characteristic
    ...
    phi_stall.append(phi_last)
    psi_stall.append(psi_last)
    ...
    plt.plot(phi_sol, psi_sol, '--k', lw = 1, label='transient')
    ...
    plt.xlim(lim_phi)
    plt.ylim(lim_psi)
    plt.savefig('rot_stall_k_%.2f.pdf' %(k_idx), bbox_inches='tight')

# plt.figure(figsize=(10, 6))
# plt.plot(phi, psi_c, label='unstalled characteristic')
# # plt.plot(phi, k_valve * phi ** 2, label='throttle characteristic')
# plt.plot(phi_stall, psi_stall, '--k', lw=1, label='transient')
# #
# # plt.scatter(phi_sol[0], psi_sol[0], c='black', marker='o', s=50, label='initial point')
# # plt.scatter(phi_sol[-1], psi_sol[-1], c='black', marker='s', s=50, label='final point')
# plt.ylabel(r'$\Psi$')
