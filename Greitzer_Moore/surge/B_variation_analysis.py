...
B_vec = np.linspace(0.2, 3, 100)

for B_idx in B_vec:

    B = B_idx  # B parameter of the described compressor, rotating stall
    ...
    plt.plot(phi_sol, psi_sol, '--k', label='transient')
    ...
    plt.ylim(lim_psi)
    plt.title('B: %.2f' %(B))
    plt.savefig('pics/surge_B_%.2f.png' %(B), bbox_inches='tight')
